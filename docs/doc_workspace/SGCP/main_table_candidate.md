...
2. FullPerception-RSU and EdgeCooper-style proxy belong to the RSU/edge-assisted layer because they use a global candidate pool; FullPerception-Decentralized proxy is the fair V2V-only counterpart.
...
- FullPerception-RSU, FullPerception-Decentralized and EdgeCooper-style explicit proxy rows are now available. The next verification step is true NS3 replay for `fullperception_decentralized`; EdgeCooper should be upgraded from per-receiver greedy blind-spot proxy to minimum-cost-flow-style global assignment before being described as a close reproduction.
