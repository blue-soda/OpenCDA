...
| FullPerception PCS (`pcs.py`) | Built-in FullPerception | 0.44 | 0.39 | 0.17 | 12,684,880 | 24.75 | Not replayed | Corresponds to paper PCS, but current implementation under-utilizes bandwidth and needs repair/calibration |
...
| FullPerception-Decentralized proxy | Fair V2V baseline | 0.80 | 0.76 | 0.41 | 38,920,592 | 75.94 | Dry-run only | Cluster-local V2V candidate pool, 3 members/head, 117 grid budget |
...
2. The repository's built-in FullPerception implementation is `pcs.py`; current `fullperception_pcs` results are weak because the engineering implementation under-schedules. FullPerception-RSU and EdgeCooper-style proxy are additional RSU/edge-assisted diagnostics because they use a global candidate pool; FullPerception-Decentralized proxy is the fair V2V-only counterpart.
...
6. SGCP coverage-aware 20ch reaches `0.80/0.76/0.41` at 73.98 Mbps and can be used as a network-resource sensitivity row: higher channel budget improves AP@0.7 but does not beat PAPG on AP@0.3/AP@0.5.
7. Target-aware PG `0.80/0.76/0.39` at 60.62 Mbps is now best treated as an ablation: it proves that moving target-aware utility into the allocator helps, while PAPG adds the missing coverage layer.
8. `B_h=2` coverage-aware sensitivity remains useful for explaining localization-quality tradeoff (`0.76/0.72/0.42` at 54.56 Mbps), but PAPG recovers AP@0.3/AP@0.5 without relying on that tradeoff.
9. `--max-upload-points-per-source 3000` confirms payload is tunable down to 38.07 Mbps, but current deterministic sampling loses AP. It is evidence for a communication knob, not a final algorithm row.
...
- Built-in FullPerception PCS is now identified and aliased as `fullperception_pcs`, but must be repaired/calibrated before it can stand as the main FullPerception baseline. FullPerception-RSU, FullPerception-Decentralized and EdgeCooper-style proxy rows remain useful diagnostics. The next verification step is PCS repair or true NS3 replay for `fullperception_decentralized`; EdgeCooper should be upgraded from per-receiver greedy blind-spot proxy to minimum-cost-flow-style global assignment before being described as a close reproduction.
