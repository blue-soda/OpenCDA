...
2. FullPerception-RSU proxy belongs to the RSU/edge-assisted layer because it uses a global candidate pool; FullPerception-Decentralized proxy is the fair V2V-only counterpart.
...
- FullPerception-RSU and FullPerception-Decentralized explicit proxy rows are now available. The next verification step is true NS3 replay for `fullperception_decentralized`; EdgeCooper proxy remains preliminary and should be refined before entering this table.
