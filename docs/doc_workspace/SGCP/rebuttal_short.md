...
The revised claim is intentionally more conservative: SGCP does not claim to dominate the centralized full-sharing upper bound. Instead, it provides a decentralized, stability-aware and channel-feasible perception-sharing pipeline. The PAPG main setting improves over forced-budget random at nearly identical traffic, improves AP@0.3/AP@0.5 over the high-budget density selective baseline while using less payload, substantially reduces communication relative to centralized full sharing, and has request-level NS3 evidence that scheduled transmissions obey the intended subchannel constraints.
