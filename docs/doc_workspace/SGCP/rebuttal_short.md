...
We thank the reviewers for the constructive comments. In the revision, we substantially tightened the experimental protocol and the claim boundary. We now separate centralized full-sharing upper references from fair RSU-free V2V baselines, replace the old main table with reproducible CARLA/OpenCDA/NS3 results, report AP together with measured upload Mbps, and add NS3 request-level delivery diagnostics. We also clarify SGCP as a perception-driven, capacity-constrained coalition and PPS framework, add a reproducible density-calibration protocol for `f(rho)`, report `rho_th`, `N_max`, and `T_min^stab` sensitivity, and soften the real-time claim to near-real-time control-plane feasibility.
...
**FullPerception fairness.** The current dump is RSU-free, so FullPerception-RSU is not used as a fair main baseline. Full 20-CAV early fusion is reported only as a centralized upper reference: `0.85/0.83/0.48` AP at `118.71 Mbps`. For fair RSU-free comparison, we add CAV-only selective-sharing baselines under the same dump, backbone, cluster-head late-fusion path, and comparable budgets. The strongest high-budget selective baseline reaches `0.80/0.76/0.40` at `73.58 Mbps`, while SGCP coverage-aware 20ch reaches `0.80/0.76/0.41` at `73.98 Mbps` with NS3-verified subchannel feasibility.
...
**Fair baselines.** Random and MWS are now treated as w/o-PPS diagnostics because their payloads are only 18.98/19.34 Mbps and they do not fully use the channel budget. The fair main comparison uses capacity-matched CAV-only selective baselines, including nearest, density-based, and communication-aware variants. This avoids mixing centralized, RSU-assisted, and RSU-free settings.

**Ablations.** We added mechanism probes: head-only late fusion gives `0.26/0.22/0.09`, SGCP grid-constrained fusion gives `0.77/0.73/0.35`, full-cluster upload gives `0.82/0.79/0.42`, random-grid same-link selection gives `0.78/0.75/0.36`, and coverage-aware 10ch `rho_th=3` gives `0.79/0.76/0.38` at `57.38 Mbps`. These results identify grid selection and channel budget as the main AP/payload drivers.
...
**NS3 reliability.** We now distinguish application callback delivery, RLC request completion, and PHY diagnostics. For SGCP coverage-aware 10ch with `rho_th=3`, the first 11 replayed frames contain 110 scheduled requests and 44 skipped unscheduled demands. All 110 scheduled requests are application-delivered and RLC-complete, with 2,970/2,970 RLC TX/RX events and zero PHY failures. In a five-subchannel exposed-bandwidth regression, requests on legal subchannels complete, while out-of-window requests are rejected before CAM/RLC creation rather than being miscounted as successful receptions.

## Claim Boundary

The revised claim is intentionally more conservative: SGCP does not claim to dominate every selective V2V heuristic in AP alone. Instead, it provides a decentralized, stability-aware and channel-feasible perception-sharing pipeline that reaches comparable or better high-IoU AP under matched V2V budgets, substantially reduces communication relative to centralized full sharing, and has request-level NS3 evidence that the scheduled transmissions obey the intended subchannel constraints.
