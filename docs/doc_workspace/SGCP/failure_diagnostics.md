...
| Avg full-reference exact-box points | 164.98 |
| Avg SGCP / full-reference exact-box point ratio | 0.62 |
| Rows with SGCP / full-reference exact-box ratio < 0.5 | 18 / 40 |
| Rows where nearest CAV is best raw exact-box source | 34 / 40 |

This refines the failure mode again: the detector miss is usually not caused by an empty target neighborhood. Most objects have target-box or near-box points in the SGCP constrained input, but the most informative nearest-CAV object points are often not directly uploaded to the nearest head. Full-reference has substantially higher object support on many rows, and the nearest CAV is usually the best raw object-support source. Therefore the next algorithmic direction should be instance-support-aware scheduling: favor sender/grid choices that preserve object shape completeness rather than only coarse grid density.

## Next Actions

1. Add target-grid diagnostics to `offline_inference --object-diagnostics-output` or a companion tool: for every missed GT, record per-head pre-NMS best IoU, score, and source CAVs. Follow-ups completed via `sgcp_head_box_diagnostics` and `sgcp_object_point_association`; next step is turning the full-reference object-support gap into an instance-support-aware PPS utility.
