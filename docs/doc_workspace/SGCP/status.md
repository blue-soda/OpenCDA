...
- 已完成 P1 protocol-native claim audit：新增 `docs\doc_workspace\SGCP\protocol_native_claim_audit.md`。当前 `main.tex` 已把 Full20Early 写成 upper reference、FullPerception-PCS 写成 repaired/tuned built-in baseline、EdgeCooper-HD 写成 edge-assisted/global assignment reference、Pure late 写成 prediction-sharing reference，并把 SGCP 的 AP@0.3 贡献明确归因到 system protocol / inter-cluster late fusion，而不是单纯 scheduler 优势。P1 剩余风险集中在 Pure late detector/checkpoint fairness 和 early-fusion checkpoint 强度。
