...
- Random-grid probe 已显示当前 utility selection 不优于随机候选；`spatial_diverse` 已超过 random-grid，且 20 子信道下 AP@0.7 接近 full-cluster upper reference。主表修复下一步应将 coverage-aware selection 固化为 SGCP 机制，并补 fixed-cluster/fixed-link、NS3-aware 丢包裁剪和论文主表候选对比。
