...
- 已将 `offline_ns3_replay` 对齐 coverage-aware 主表候选，支持 `--num-channels`、`--bandwidth-mhz`、`--sgcp-grid-score-mode` 和 `--sgcp-grid-selection-mode`。20 子信道 `spatial_diverse` 11 帧 NS3 replay 完成：154/154 scheduled request application callback delivered，RLC complete 154/154，`MANUAL_CMD_REJECT=0`，PSCCH/PSSCH decode failure 均为 0。
...
- Random-grid probe 已显示当前 utility selection 不优于随机候选；`spatial_diverse` 已超过 random-grid，且 20 子信道下 AP@0.7 接近 full-cluster upper reference。20 子信道 high-budget 候选的 NS3 request-level delivery 已验证。主表修复下一步应将 coverage-aware selection 固化为 SGCP 机制，并补 fixed-cluster/fixed-link、10 子信道 NS3 replay 或论文主表候选对比。
