...
- 已新增 `--num-channels` / `--bandwidth-mhz` 离线参数，并完成网络资源敏感性第一版：5/10/20 子信道 AP@0.5 分别为 0.53/0.73/0.73，平均上传 payload 分别为 60,225/109,415/139,300 bytes/source；20/40/80 MHz 当前结果完全一致，需复核 PPS 带宽模型。
...
- 网络资源实验已证明子信道数量影响 PPS 结果；但总带宽覆盖在当前离线路径中没有产生差异，论文级实验前需要检查 `bandwidth_all/bandwidth_per_channel` 在 `PotentialGame` 吞吐约束中的作用。
