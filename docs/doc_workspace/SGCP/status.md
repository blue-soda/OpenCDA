...
- 已新增 `opencda.tools.sgcp_object_point_association`，对 PAPG dense-miss top40 统计 SGCP constrained input 中真正落入 GT BEV box/邻域的点数。结果：exact box 内仅 1/40 行完全无点，2m 邻域 0/40 行无点；但 31/40 行 nearest CAV 的 exact-box 点没有直接上传到 nearest head。结论：剩余漏检不是简单“没有点”，而是 coarse grid coverage 无法保证实例级形状支撑；下一步应比较 full-reference 与 SGCP 的 object/near-object support gap，并设计 instance-support-aware scheduling。
