...
- EdgeCooper 已从逐 receiver blind-spot proxy 升级为 `edgecooper_global` network-aware proxy：加入全局 sender 负载均衡和 35 m V2V 可行性门控后，41 帧结果为 `0.81/0.77/0.42`、38,223,408 bytes / `74.58 Mbps`。11 帧真实 NS3 replay 为 73/110 application/RLC complete、0 PHY failures，说明离线 AP proxy 较强，但 deadline delivery 还不如 PAPG 的 110/110 complete；该行应归入 RSU/edge-assisted 诊断/补充 baseline，不替代公平 V2V 主线。
