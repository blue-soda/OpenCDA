...
- 已新增 network-level satisfaction first metric：`opencda.tools.sgcp_satisfaction_summary` 从 `offline_inference --object-diagnostics-output` 汇总 receiver-frame 的 full-reference-detectable GT recovery，并输出 satisfaction rate。`tau=0.85` 在已有 41 帧结果中具有区分度：Spatial-diverse 10ch mean recovery / satisfaction@0.85 为 `0.877/0.707`，Target-aware PG 为 `0.885/0.756`，PAPG 为 `0.924/0.927`。该指标可支撑 Figure 2 satisfaction / coverage distribution；下一步为 protocol-native baselines 统一生成 object diagnostics。
