...
- 在线 CARLA-NS3 时间同步修复已通过真实 35 tick 图形短回归，但该回归仍观察到大量 PHY decode failures 和部分 large upload incomplete；这更像在线真实 PHY/并发/分片压力问题，不影响“时间基准已对齐”的结论，但若要把在线 NS3 作为论文主结果，还需逐 request 解析 application/RLC/PHY。
