...
for name in ['sgcp_default_ns3_replay_job_abs','sgcp_high_capacity_ns3_replay_job_abs','sgcp_high_mcs_symbols_ns3_replay_job_abs','sgcp_high_mcs_symbols_rri5_ns3_replay_job_abs']:
    ...
    fatal='NS_FATAL' in text or 'SIGABRT' in text or 'Invalid number of RBs' in text
    out={'variant':name,'fatal':fatal,'manual_add':len(re.findall(r'\[MANUAL_CMD_ADD\]', text)),'cam_received':len(rx),'consume_events':len(vals)}
    if vals:
