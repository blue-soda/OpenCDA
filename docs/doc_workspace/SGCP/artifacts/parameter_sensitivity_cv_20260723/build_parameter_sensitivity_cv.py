# -*- coding: utf-8 -*-
"""Run SGCP C/V parameter sensitivity around the formal operating point."""
...
ARTIFACT = REPO / "docs/doc_workspace/SGCP/artifacts/parameter_sensitivity_cv_20260723"
...
def write_csv(rows):
    path = ARTIFACT / "parameter_sensitivity_cv_20260723.csv"
...
def run_compute_profile(metrics_csv):
    ...
    output_csv = ARTIFACT / "parameter_sensitivity_cv_compute_profile_20260723.csv"
    summary_md = ARTIFACT / "parameter_sensitivity_cv_compute_profile_20260723.md"
    cmd = [
...
def merge_compute(rows, compute_csv):
    ...
    path = ARTIFACT / "parameter_sensitivity_cv_with_gflops_20260723.csv"
    with path.open("w", newline="", encoding="utf-8") as stream:
        writer = csv.DictWriter(stream, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)
...
def write_markdown(rows):
    ...
    for group, title in groups:
        ...
        lines.append("")
    output = ARTIFACT / "parameter_sensitivity_cv_20260723.md"
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(output)
    return output
