...
| Cluster-local selective proxy | 0.80 / 0.76 / 0.41 | 38,920,592 bytes | cluster-local V2V candidates, 3 members/head, 117 grid budget; NS3 110/110 complete |
...
- 当前 selective proxy 代码已接入 `offline_ns3_replay --selective-sharing-baseline`。`cluster_local_selective_proxy` 的 11-frame true NS3 replay 已完成：110/110 application callback complete、110/110 RLC complete、RLC drops=0、PHY decode failures=0，使其链路证据与 PAPG/forced random 对称。
