...
- [ ] 继续调优 `object_aware_potential_game`：已新增逐帧 target-grid case study 和 OAPG 分支，确认 frame 000068/object 438、frame 000066/object 401、frame 000084/object 350 等最佳视角未调度问题可被 sender refinement 修复；但 11 帧快速 AP 仅为 `0.74/0.69/0.30`，尚未优于当前主表候选。下一步做 41 帧完整评估、每 head 最多替换一个 RB / detector-quality gate，并 dump 已选中 target-grid 仍漏检对象的 pre-NMS boxes。
- [ ] 补 `target_aware_potential_game` 真实 NS3 socket replay：基于已生成的 11 帧 dry-run upload plan，启动 NS3 后验证 scheduled request application/RLC complete、manual subchannel 无 reject、PHY failure 为 0 或可解释。
