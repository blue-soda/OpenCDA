...
- [ ] 分析真实在线 NS3 中的大包分片与 PHY decode failure：第一轮日志诊断发现 `PotentialGame.clear_resource_allocation_strategy()` 未清空各 CAV `ClusteringScheduler.channel_allocation`，导致在线多轮 CP 残留旧链路；已补清理逻辑并通过真实短回归验证主要 PHY collision 大幅下降。已新增 `opencda.tools.online_ns3_log_eval` 做 episode-level 解析，确认最新回归中 184 行 incomplete 实际对应 6 个 partial episode，且每个都缺 1 个 10000-byte fragment。下一步需要实现 fragment-level retransmission/drop-aware fusion，或补更长 tick/drain 与逐 request RLC/PHY 对齐。
