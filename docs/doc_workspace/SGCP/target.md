...
- [ ] 执行真实 CARLA+NS3 有限 tick 短回归：已新增 `online_ns3_short_regression.md`，明确先启动 WSL ns-3、再启动 CARLA、最后运行 `opencda.py ... --network`；本轮已通过本地 `network_time_sync` 轻量测试并确认无残留进程，下一步补实际 stdout/log 和验收结果。
