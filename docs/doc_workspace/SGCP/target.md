...
- [x] 修正主表 baseline 口径：当前 RSU-free dump 上不可直接填真实 RSU-sensor reference；full 20-CAV early `0.85/0.83/0.48`、60,838,528 bytes 只能作为 centralized full-sharing upper reference，不能命名为 FullPerception。FullPerception baseline 对应仓库 `pcs.py` / `fullperception_pcs`，本轮 tuned PCS 结果为 `0.59/0.53/0.22`、12,959,840 bytes / 25.29 Mbps。Random/MWS scheduler 因 payload 过低只作 w/o PPS 消融；公平主对比使用 payload-matched selective sharing 和 SGCP PAPG。此前两个后补 proxy 已重命名为 `global_selective_proxy` / `cluster_local_selective_proxy`，避免误导。
...
- [x] 核查 FullPerception baseline：不要再把 full 20-CAV early fusion 混写为 FullPerception。已确认 `pcs.py` 是 FullPerception 论文 PCS 内置实现，正规入口为 `fullperception_pcs`；当前 tuned PCS 41 帧结果为 `0.59/0.53/0.22`、12,959,840 bytes / 25.29 Mbps，和 full 20-CAV upper reference `0.85/0.83/0.48`、118.71 Mbps 分开记录。
- [x] 重命名后补 selective proxy：旧 `fullperception_rsu` 已改为 `global_selective_proxy`，旧 `fullperception_decentralized` 已改为 `cluster_local_selective_proxy`。二者只作为 global/cluster-local selective proxy 或 diagnostic，不再作为 FullPerception 论文复现命名。重命名前的 41 帧结果分别为 `0.84/0.80/0.46`、109.71 Mbps 和 `0.80/0.76/0.41`、75.94 Mbps；cluster-local proxy 的 11 帧真实 NS3 replay 为 110/110 application/RLC complete、0 PHY failures。
- [x] 复现 EdgeCooper baseline：参考本地论文 `C:\Users\sakakibara\OneDrive\Papers\Cooperative Perception\EdgeCooper_Network-Aware_Cooperative_LiDAR_Perception_for_Enhanced_Vehicular_Awareness.pdf`，实现 edge/virtual-RSU assisted complementarity-enhanced、redundancy-minimized raw LiDAR scheduling proxy；优先使用同一 20MHz/10ch scheduled-only NS3 口径验证。已完成 blind-spot-aware first proxy，41 帧结果 `0.75/0.70/0.32`、56,134,048 bytes / 109.53 Mbps；已进一步新增 `edgecooper_global`，用全局 sender 负载均衡和 35 m V2V 可行性门控近似 edge-side global assignment，41 帧达到 `0.81/0.77/0.42`、38,223,408 bytes / 74.58 Mbps，但 11 帧 NS3 replay 只有 73/110 complete。现已新增 `edgecooper_global_hd` 半双工约束，禁止同一 slot 内 receiver 同时作为 sender，41 帧达到 `0.81/0.78/0.42`、33,519,040 bytes / 65.40 Mbps，11 帧 NS3 replay 为 110/110 application/RLC complete、0 PHY failures。后续若要更贴近原论文，可继续实现 MCF/conflict-coloring，但当前已足以作为 RSU/edge-assisted 强 baseline。
- [ ] 应对 EdgeCooper-HD 对主表叙事的新压力：若主表混放 edge-assisted 与 V2V-only，PAPG 不再是 AP@0.7 最高；下一步要么将主表显式分层为 RSU/edge-assisted 与 V2V-only decentralized，要么继续提升 PAPG 高 IoU AP，同时保持 62 Mbps 左右和 110/110 NS3 delivery。
- [x] 第一轮应对 EdgeCooper-HD 压力：已更新 `C:\Workspace\icdcs-paper\SGCP\main.tex`，把 EdgeCooper-HD 写成 edge-assisted reference 而非 V2V-only 公平 baseline；同时完成 PAPG `B_h=3` 负面 probe，结果 `0.80/0.78/0.40`、62.54 Mbps，说明简单放宽 per-head RB 上限无法追平 EdgeCooper-HD，需要后续做高质量 source / target-grid coverage 保护。
- [x] 第二轮应对 EdgeCooper-HD 压力：新增 `balanced_perception_aware_potential_game` / `bpapg`，把 source-diversity marginal term 写入 PAPG 两层势函数，并试验跨帧 source-history credit。41 帧 source-balanced 结果仍为 `0.81/0.78/0.39`、62.54 Mbps，per-CAV upload distribution 与 PAPG 相同；11 帧 history-credit 降为 `0.75/0.71/0.33`。结论：朴素欠服务源车轮换不是有效主线，后续必须以 target-quality / detector-quality 为门控保护关键低频 source。
- [x] 第三轮应对 EdgeCooper-HD 压力：新增 `quality_gated_perception_aware_potential_game` 与 `head_urgent_perception_aware_potential_game`。QG-PAPG 11 帧 `0.75/0.72/0.33`，说明 quality gate 下的 source-history 仍会伤 AP；HU-PAPG 41 帧 `0.81/0.78/0.39`、62.54 Mbps，安全但无增益，`B_h=3` 11 帧也不变。结论：停止继续调 source/head fairness 系数，下一步转向 detector/pre-NMS 级诊断，定位 dense target grid 已送达但无 final box 的失败链。
- [x] 新增 detector/pre-late-fusion 诊断：针对 failure_diagnostics 中 “nearest head got dense points but no final box” 的 bucket，新增 `opencda.tools.sgcp_head_box_diagnostics`。PAPG dense-miss top40 结果为 nearest-head matched 0/40、any-head matched 0/40、late-fused matched 0/40、full-reference matched 40/40，说明问题不是 late fusion/NMS 抑制已有正确框，而是 head-local detector 没有对目标出框。
- [x] 新增 object-level point association：对 dense-miss top40 统计 selected grid 内有多少上传点真正落入 GT 3D box 或其邻域，区分“grid 有点但目标无点/点太少”和“目标有点但 detector 仍无框”。已新增 `opencda.tools.sgcp_object_point_association`：PAPG dense-miss top40 中 exact GT BEV box 内 39/40 行已有 SGCP 点，2m 邻域 40/40 行都有点，但 31/40 行 nearest CAV 的 exact-box 点未直接上传到 nearest head；下一步转向 full-reference vs SGCP object-support gap 和 instance-support-aware scheduling。
- [x] 比较 full-reference 与 SGCP 在漏检 GT box/near-box 内的点数差距：第一轮 dense-miss top40 中，full-reference exact-box 平均 164.98 点，SGCP/full exact-box ratio 平均 0.62，18/40 行低于 0.5；nearest CAV 在 34/40 行是最佳 raw object-support source。结论：full-reference 的优势主要来自更完整的实例级点云支撑和关键近距离视角，而不是单纯更多 coarse-grid 点。
- [x] 设计 instance-support-aware PPS utility：已新增 `instance_support_potential_game` (`ispg`)，在 PAPG coverage layer / target layer 内加入紧凑高密度 component、weak-head gain 和 unique-best-view gain。41 帧 20MHz/10ch/`rho_th=3`/`B_h=2` 结果为 `0.80/0.78/0.39`、32,046,336 bytes / `62.53 Mbps`，与 PAPG 主参考基本持平但没有提升；trace 显示覆盖结构仍为每帧 16/20 fused、10/20 uploaded、4/20 unscheduled。
- [x] 设计 target-to-head routing / cross-cluster instance-support mechanism first probe：已新增 `cross_cluster_instance_support_potential_game` (`ccispg`)。Naive 版 11 帧 external links 为 104/110，AP `0.68/0.64/0.37`；layered 版 external 44/110，AP `0.75/0.71/0.33`；cap1 版 external 11/110，AP `0.75/0.72/0.33`。结论：跨簇路由能力可用，但自动 external sender 会破坏 coverage，不能直接作为主方法。
- [x] 设计 diagnostic-triggered / persistent-target-triggered cross-cluster routing first oracle probe：已新增 `offline_inference --sgcp-routing-hints-csv`。11 帧 PAPG + dense-miss top40 routing hints 触发 9 次 frame-level replacement，external links 2/110，结果 `0.75/0.71/0.35`、8,521,936 bytes / `61.98 Mbps`；AP@0.7 小幅上升但 AP@0.3/AP@0.5 下降，说明 object-support gap 不是充分触发条件。
- [x] 设计 context-preservation routing trigger first probe：已将 routing hints 的已调度 sender 行为改成 merge replacement，最多插入 3 个 object-neighborhood grids，其余保留原高密度上下文 grids。11 帧结果仍为 `0.75/0.71/0.35`、8,563,440 bytes / `62.28 Mbps`，说明 object-grid hint 的低阈值下降不是简单 whole-list replacement 造成。
- [x] 完成 detector-benefit post-hoc 对比：11 帧 PAPG 无 hint 基线与 merged routing hints 逐 GT 对比显示，routing hints 修复 4 个 GT，但打掉 15 个原本命中的 GT；PAPG `full_detected_method_missed=82`，hint 为 91。结论：只看 object-support gap 或总 pred/GT context 不足，必须保护已覆盖 object prototypes。
- [ ] 若继续算法改造，设计 proposal/objectness-level replacement trigger：触发前估计候选 route 是否新增 object prototype，且不移除当前已覆盖 object prototype；若短评估仍负面，则停止 routing probe 扩张，转向 PAPG 主线论文收敛和更强 baseline 分层叙事。
- [x] 修复/校准内置 FullPerception PCS：已确认 `pcs.py` 对应原论文 PCS，并修复 `c(q)=1` 死代码、`--bandwidth-mhz/--num-channels` 未进入 required subchannels、`sc_num` 未传入 NS3 replay、PCS 全局 scheduled links 被 coalition cluster 过滤、receiver policy 只能固定为 cluster head、blind-spot cache 忽略 split 粒度、grid mAP cache 索引错误、utility/payload/grid-selection 使用不同 blind-spot 粒度等问题。当前默认 `division=12,min_overlap=0`，41 帧 `fullperception_pcs` scheduled-receiver 结果为 `0.59/0.53/0.22`、12,959,840 bytes / 25.29 Mbps；11 帧 NS3 dry-run 每帧 5 scheduled requests、0 skipped unscheduled。
- [x] 继续复核 FullPerception MWS/RS 是否也应使用相同 blind-spot 粒度或保留论文 heuristic 默认参数：已将 MWS/RS 改为复用 tuned PCS blind-spot 粒度、记录 `sc_num` 并只上传 sender 覆盖 grids。11 帧 sanity 结果：MWS `0.36/0.32/0.15`、39.00 Mbps；RS/random `0.54/0.49/0.23`、14.95 Mbps。结论是二者仍只适合作 w/o-PCS heuristic 诊断，不进入主公平表。
- [x] 搜索并选择若干最新且适合作为审稿回复的 decentralized / V2V-only collaborative perception baselines，优先考虑 Where2comm/PACP/What2comm/CoBEVT/V2VNet 中可用当前 dump 和 OpenCOOD backbone 复现或近似实现的机制；每个 baseline 必须说明是否为真实复现、proxy 复现或不适合当前数据/模型。已新增 `baseline_reproduction_plan.md`；已核实 PACP 原文是 RGB/BEV 方法而非点云原生，并实现 `pacp_lidar` 作为 PACP-style LiDAR priority proxy：高预算 41 帧 `0.81/0.79/0.42`、86.56 Mbps，低预算 `0.76/0.73/0.37`、67.31 Mbps。下一步若继续补近年 V2V baseline，优先 Where2comm-style confidence communication 或严格 checkpoint 复现。
- [x] 重构主表 baseline 分层：AP upper bound（full 20-CAV early）、正式 FullPerception PCS baseline、RSU/edge-assisted proxy（global selective proxy、EdgeCooper）、V2V-only decentralized baselines（cluster-local selective proxy、forced random、density/communication-aware、selected SOTA proxy）和 SGCP PAPG 分开呈现。已同步 `results.md`、`baseline_fairness.md`、`fullperception_baseline_revision.md` 和 `main_table_candidate.md`。
...
- [x] 补充 FullPerception baseline 的实现细节和公平性讨论。已新增并更新 `fullperception_baseline_revision.md`，明确 `pcs.py` 是正式 FullPerception PCS baseline；full 20-CAV fusion 只能作为 centralized upper reference，公平主对比应使用 same-budget CAV-only selective-sharing baseline。
