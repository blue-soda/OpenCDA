...
- [ ] 用真实 CARLA + NS3 或离线 NS3 replay 验证时间同步和 transfer request 语义：CARLA tick / OpenCDA network time slot / NS3 sync time 必须一致；带宽内无冲突请求成功，带宽外请求延迟或丢包。已完成 default 10/5 子信道回归和 20 子信道 `spatial_diverse` high-budget replay：154/154 request application/RLC complete，PHY failures 0；后续可补 10 子信道 `spatial_diverse` 同口径 replay 或真实 CARLA+NS3 短回归。
