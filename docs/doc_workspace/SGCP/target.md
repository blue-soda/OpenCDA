...
- [ ] 设计 detector-benefit routing trigger：在触发 route replacement 前评估候选 route 是否可能提升 head-local objectness / pred-GT context，而不是仅依赖 object-support gap；若该方向继续负面，应停止 routing probe 扩张，转向 PAPG 主线论文收敛和更强 baseline 分层叙事。
