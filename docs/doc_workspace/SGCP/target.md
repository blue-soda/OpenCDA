...
- [ ] 设计 target-to-head routing / cross-cluster instance-support mechanism：将 object-support proxy 从簇内 sender/grid utility 提升到 receiver/head assignment 层，允许关键实例视角跨簇送到最相关 head，目标是在不增加 10ch/`B_h=2` 总预算的前提下减少 persistent full-reference-detected misses。
