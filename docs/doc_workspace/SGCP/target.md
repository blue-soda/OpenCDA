...
- [ ] 分析真实在线 NS3 中的大包分片与 PHY decode failure：第一轮日志诊断发现 `PotentialGame.clear_resource_allocation_strategy()` 未清空各 CAV `ClusteringScheduler.channel_allocation`，导致在线多轮 CP 残留旧链路，新旧链路可能在同一 receiver/subchannel 上重叠；已补清理逻辑并通过真实短回归验证主要 PHY collision 大幅下降。下一步需要补更长 tick/drain 或在线 request-level trace，区分剩余 incomplete upload 是短窗口未 drain、少量 PHY failure，还是仍有跨轮调度语义问题。
