...
- [x] 修正主表 baseline 口径：FullPerception-RSU 在当前 RSU-free dump 上不可直接填实测；full 20-CAV early `0.85/0.83/0.48`、60,838,528 bytes 只能作为 centralized full-sharing upper reference，不能命名为 FullPerception。FullPerception baseline 对应仓库 `pcs.py` / `fullperception_pcs`，修复后协议正确结果为 `0.33/0.29/0.14`、8,100,112 bytes。Random/MWS scheduler 因 payload 过低只作 w/o PPS 消融；公平主对比使用 payload-matched selective sharing 和 SGCP PAPG。
