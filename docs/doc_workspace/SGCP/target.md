...
- [ ] 设计 detector-benefit / context-preservation routing trigger：在触发 route replacement 前评估被替换 sender 是否是稳定 coverage source、receiver 的 fused GT/pred context 是否会下降、候选 route 是否可能提升 head-local objectness；目标是在保住 PAPG `0.81/0.78` 低阈值 AP 的前提下提升 AP@0.7。
