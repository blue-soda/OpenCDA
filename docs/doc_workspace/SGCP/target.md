...
- [x] 为论文证明需求新增 exact potential-verified coalition admission variant：`potential_verified_cov_coalition_game.py`。该版本将迁移提交条件增强为 source/target affected potential increment `Delta Phi_C > 0`，可证明每个 accepted migration 满足 `J_i^C > 0` 并保证有限 partition space 下终止。41 帧 current-protocol 结果 `0.84/0.74/0.31`、raw `59.17 Mbps`；与正式 SGCP-CV 在 `40/41` 帧分簇不同，因此作为 proof-oriented variant 记录，不直接替换主表。
