...
- [ ] 设计 diagnostic-triggered / persistent-target-triggered cross-cluster routing：仅当目标区域满足“full-reference 可检、PAPG 漏检、best raw source 不在相关 head 输入中”的诊断条件时，才用 1 条 target-layer RB 做跨簇实例补偿，并保持 20MHz/10ch/`B_h=2` 总预算。
