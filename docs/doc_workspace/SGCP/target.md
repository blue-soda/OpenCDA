...
- [x] 将所有后续 FullPerception-PCS 实验固定为 raw-LiDAR adaptation 参数 `div4/radius4/min128`，并重跑/刷新 Table 1 PCS、Table 2/6 PCS + global box aggregation、Table 3/A 中 SGCP-scaffold PCS。2026-07-21 已将 `pcs.py` 默认值改为 `blind_spot_min_division=4`、`blind_spot_adjacency_radius=4`、`blind_spot_min_grids=128`；2026-07-23 已补齐 K=1/2 与 GFLOPs 对齐结果：`table1_k_sensitivity_protocol_native_20260722.csv`、`table6_global_box_k_sensitivity_current_protocol_20260723.csv`、`table3_scheduler_k_sensitivity_current_protocol_20260723.csv`、`tableA_kaware_current_protocol_diagnostic_20260723.csv`。旧 PCS 数字若未显式标注该参数，只能作为 archived diagnostic。
- [x] 核查并记录 PCS / EdgeCooper 的原始调度目标与 SGCP 实验协议差异。结论：PCS 与 EdgeCooper 均以通信约束下的感知提升为主目标，不是单纯压缩 Mbps；二者原系统都包含链路/信道/packet 或多时隙调度语义，未必等价于 SGCP 当前“一帧一次 100ms raw-LiDAR replay”。已新增 `pcs_edgecooper_protocol_alignment.md`。
- [x] 实现并验证 PCS 一帧内 repeated-round deadline admission，注意不使用 `multi-slot` 说法。2026-07-21 已新增 `--pcs-frame-rounds` / `--pcs-frame-deadline-ms`，11 帧 `div4/radius4/min128, rounds=6, deadline=60ms` 为 AP `0.22/0.17/0.07`、`11.99 Mbps`、每帧 `60.00 ms`；原始单轮 frame `000060` 需 `245.696 ms`，因此 60ms 下只能接受 deadline-cropped 第一轮。SGCP/EdgeCooperHD exact-payload 离线估算分别约 `320/327 ms`，但已有 10,000-byte scheduled-request NS3 replay 为 avg/p95 `23.91/24.00 ms`；后续若要论文强声称全 raw payload 60/100ms 完成，需要 exact-payload NS3 replay 或 PHY throughput 校准。
...
- [x] 在 paper-facing 40MHz/10ch/60ms 设置下重测 protocol-native PCS 与 EdgeCooper。PCS 单轮为 AP `0.23/0.17/0.06`、`53.55 Mbps`，NS3 `77/77` callbacks、avg/max `25.71/54.00 ms`，作为正式 PCS 行；PCS repeated-round 虽离线填满 60ms，但 NS3 sequential replay `67/97` callbacks、frame-start max `242ms`，仅保留为诊断。EdgeCooper V2V 为 AP `0.54/0.48/0.25`、`275.94 Mbps`，NS3 `15/348` callbacks、avg/max `127.87/215.00 ms`，应标注为 deadline-infeasible offline protocol adaptation。
- [x] 修正 EdgeCooper V2V deadline 约束，禁止使用超 60ms 的原版适配行作为正式 baseline。2026-07-21 已将 selective baseline 的 deadline admission 改为整帧全局预算，并对 `edgecooper_global` 增加无端点冲突 matching。最终 constrained EdgeCooper V2V：AP `0.32/0.26/0.10`，`50.91 Mbps`，NS3 `68/68` callbacks，avg/max delay `25.90/54.00 ms`。旧 `0.54/0.48/0.25, 275.94 Mbps` 行仅作 deadline-infeasible diagnostic。
...
- [x] 在当前 `Fixed first-frame clusters` 之外，补充分簇算法 baseline：最终 paper-facing Table 4 保留 3 个启发式 baseline（Random balanced、Distance-greedy、Density/quality-greedy）和 2 个正式论文映射 baseline（SeAC-inspired、HHOCNET-inspired），所有 baseline 使用同一 attentive checkpoint、同一 `cov_potential_game` C->V 调度、同一 inter-cluster box NMS、同一 40MHz/10 target-subchannel/NS3-calibrated 通信口径，只替换 clustering 维度。2026-07-24 已完成 41 帧重跑：SGCP-CV `0.87/0.80/0.36, 60.90 Mbps`；Random `0.79/0.64/0.30, 55.81 Mbps`；Distance `0.80/0.69/0.35, 56.90 Mbps`；Density `0.73/0.62/0.29, 47.00 Mbps`；SeAC-inspired `0.82/0.72/0.36, 56.93 Mbps`；HHOCNET-inspired `0.79/0.68/0.32, 55.28 Mbps`。
- [x] 调研近年 V2X / cooperative perception / vehicular edge sensing 中可复现的分簇或 coalition baseline，优先选择 1--2 个能映射到当前 raw-LiDAR V2V 场景的方法，并记录论文来源、机制映射和不适配点。最终选用 Akbar et al. `SeAC: SDN-Enabled Adaptive Clustering Technique for Social-Aware Internet of Vehicles`（IEEE T-ITS 2023, DOI `10.1109/TITS.2023.3237321`）和 Ali et al. `Harris Hawks Optimization-Based Clustering Algorithm for Vehicular Ad-Hoc Networks`（IEEE T-ITS 2023, DOI `10.1109/TITS.2023.3257484`）；DCA 2025 因仅为 arXiv preprint，未进入最终 Table 4。
- [x] 新增分簇算法消融表：最终外部 clean package 已更新 `C:\Workspace\2026-7-papers\infocom\SGCP\experiment\04_clustering_ablation.md`；OpenCDA artifact 为 `docs/doc_workspace/SGCP/artifacts/table4_clustering_cv_20260724/`。该表不再混入旧 `perception_aware_potential_game` scaffold 行，且没有 baseline 在 AP@0.3/AP@0.5 上明显超过 SGCP-CV。
...
- [x] Table 3 / Figure 4：SGCP-compatible scheduler comparison。2026-07-24 已按正式 C/V 口径重跑 scheduler rows：attentive detector、`cov_coalition_game`、all cluster heads、inter-cluster NMS、40MHz/10 target subchannels、NS3-calibrated estimator、K=2 baseline receiver concurrency。SGCP-CV 使用 Raw LiDAR Mbps Budget sweep 的 selected main row `0.87/0.80/0.36, 60.90 Mbps, 536.92 GFLOPs/frame`；Table 3 已更新到 `C:\Workspace\2026-7-papers\infocom\SGCP\experiment\03_scheduler_comparison.md`。AP@0.7 保留，Samples 与 admission-budget columns 不汇报。
- [ ] Table 2 / Figure 3：fusion scaffold ablation，至少包含 Head-only、Pure late、Clustered early-only、Full SGCP、Full 20-CAV early reference。2026-07-22 已完成 current-protocol diagnostic rerun，并生成 `table2_fusion_scaffold_current_protocol_20260722.csv` 与 `figure3_fusion_ablation_current_protocol_20260722`；但 Pure late `0.82/0.76/0.37, 0.74 Mbps` 明显强于严格 60ms 下的 Full SGCP `0.64/0.60/0.25, 37.05 Mbps`，因此不能作为 paper-facing final fusion ablation。后续需与 Table3 一并决定 PAPG operating point / raw-LiDAR budget / pure-late 叙事边界。
- [x] Table 4 / clustering ablation：保持 `cov_potential_game` C->V scheduler + inter-cluster NMS，只替换 clustering。2026-07-24 已重跑并替换 clean package `04_clustering_ablation.md`，最终保留 3 个启发式 baseline 和 2 个论文 baseline；旧 Table5/Figure7 current-protocol diagnostic 不再作为 paper-facing final clustering ablation。
- [ ] Table 4 / Table 4b / Figure 5：rho/channel/Nmax sensitivity。2026-07-22 已完成 current-protocol diagnostic rerun，并生成 `table4_parameter_sensitivity_current_protocol_20260722.csv` 与 `figure5_parameter_sensitivity_current_protocol_20260722`；但结果显示 `N_max=2` 为 `0.82/0.77/0.39, 60.13 Mbps`，明显强于默认 `N_max=4` strict-budget PAPG `0.64/0.60/0.25, 37.05 Mbps`，且 target-subchannel sweep 有意改变 `num_channels`。因此该表只能作为参数诊断，不能直接支撑旧默认参数叙事；后续需决定是否把主线 operating point 改为 `N_max=2`、重跑相关图表，或将该表转入 appendix diagnostic。
- [ ] Table 6 / Figure 8：global box aggregation normalized baselines。2026-07-22 已完成 current-protocol diagnostic rerun，并生成 `table6_global_box_aggregation_current_protocol_20260722.csv` 与 `figure8_global_box_aggregation_current_protocol_20260722`；Pure late 为 `0.82/0.76/0.37, 0.74 Mbps`，PCS+global-box 为 `0.83/0.77/0.38, 54.54 Mbps`，EdgeCooper+global-box 为 `0.84/0.79/0.37, 51.83 Mbps`，strict SGCP default 为 `0.64/0.60/0.25, 37.05 Mbps`。该表确认 common global box aggregation 会显著抬高 baseline，不能直接作为最终 Table6 支撑 strict default SGCP；后续需在 `N_max=2` 或其他 SGCP operating point 决策后重算/重组。
- [ ] Table A / Figures 1-2：只在上述组件表稳定后重新合成。2026-07-22 已生成 current-protocol diagnostic synthesis：`tableA_combined_current_protocol_diagnostic_20260722.csv`、`tableA_compact_current_protocol_diagnostic_20260722.csv`、`figure1_pareto_current_protocol_diagnostic_20260722` 与 `figure2_combined_current_protocol_diagnostic_20260722`；但该 synthesis 混合 references、normalized global-box scaffolds、scheduler diagnostics 和 unresolved SGCP operating points（strict default / `N_max=2` / cap4000），只能作为诊断视图，不能作为最终 paper-facing TableA/Figure1/2。
- [ ] Figure 6：bootstrap uncertainty，等待最终 current-protocol 行冻结后重算。2026-07-22 已新增 `figure6_current_protocol_status_20260722.md`，明确现有 `uncertainty_bootstrap_attentive.csv` 是 legacy 20MHz，不能作为 current-protocol evidence；最终行冻结并生成 per-sample eval stats 后再重算。

验收标准：

- [x] 每张已进入 clean package 的主表 caption/source registry 必须明确 `checkpoint`、`bandwidth_mhz`、`num_channels`、`communication_deadline_ms`、`late_fusion`、`clustering`、`resource_allocation`。2026-07-24 已检查 Table3/Table4；Table4 明确 attentive、40MHz/10ch、NS3 estimator、`cov_potential_game`、inter-cluster NMS 和逐行 clustering。
...
- [x] 使用 attentive checkpoint 在 frame `000060` 上完成真实 detector forward 校准：singleton forward `89.411751 GFLOPs`，其中 point-feature subtotal `0.061198 GFLOPs`；full 20-CAV forward `90.297247 GFLOPs`，其中 point-feature subtotal `0.946693 GFLOPs`。当前口径包含 Conv2d/ConvTranspose2d/Linear/BatchNorm/ReLU hook 和 `PillarVFE` cluster/center feature 构造、mask、PFN 前处理近似；voxelization/hash/scatter 内存/索引操作仍排除。结果显示点云量已影响统计，但 PointPillar/attentive detector 计算仍主要由固定 BEV backbone 主导。
- [x] 生成 current-protocol compute profile：`docs/doc_workspace/SGCP/artifacts/compute_profile_20260722/compute_profile_current_protocol_20260722.csv`。关键结果：Pure late 约 `19.17` detector forwards/frame、point-feature `1.17 GFLOPs/frame`、total `1714.08 GFLOPs/frame`；SGCP-PAPG 约 `6.00` forwards/frame、point-feature `0.83 GFLOPs/frame`、total `536.94 GFLOPs/frame`；no-clustering EdgeCooper/PCS global-box 约 `19-20` forwards/frame、total `1718.82-1775.54 GFLOPs/frame`。
- [x] 生成 protocol-native compute profile：`docs/doc_workspace/SGCP/artifacts/compute_profile_20260722/compute_profile_protocol_native_20260722.csv`。Table1 PCS/EdgeCooper no-late 原版适配均约 `20` detector forwards/frame，PCS total `1788.63 GFLOPs/frame`、deadline-constrained EdgeCooper total `1788.61 GFLOPs/frame`，但输入点数和通信负载不同。
...
- [x] 将 GFLOPs 合并进 paper-facing 源表：`table1_original_protocol_baselines_20260720.csv`、`table2_fusion_scaffold_current_protocol_20260722.csv`、`table3_scheduler_comparison_current_protocol_20260722.csv`、`table5_clustering_ablation_current_protocol_20260722.csv`、`table6_global_box_aggregation_current_protocol_20260722.csv` 均新增 `point_feature_gflops_per_frame`、`detector_gflops_per_frame`、`gflops_note`。同时新增/同步 `compute_profile_clustering_current_protocol_20260722.csv`，并修正 protocol-native compute profile，使 EdgeCooper 使用 deadline-constrained trace。
- [x] Table 1 增加 `No collaboration` 下界行，并重命名旧 `Full 20-CAV early fusion` 为 `Centralized all-in-one raw-LiDAR early fusion upper reference`。澄清：前者是 820 个 singleton receiver-frame、本地检测、0 Mbps；后者是 41 个 all-in-one fused receiver-frame 的 centralized upper reference，不是 protocol-native all-receiver full-broadcast baseline。真实 all-receiver full early broadcast 在当前 `40 MHz / 10 target subchannels / 60 ms` 约束下视为通信不可行，不作为 feasible baseline。
- [x] 按用户要求保留当前 Table 1 默认结果，并为 PCS / EdgeCooper 增加 receiver concurrent-inbound 参数 `K`。默认 `K=1` 保持当前 Table 1；`K=2` 只放宽“同一接收方最多同时接收几个正交子信道发送方”，sender half-duplex、deadline admission 和调度目标不变。新增实验结果：PCS K=2 为 `0.31/0.25/0.11, 49.97 Mbps, 1788.60 GFLOPs/frame`；EdgeCooper K=2 为 `0.34/0.27/0.11, 60.34 Mbps, 1788.68 GFLOPs/frame`。结果已写入 `docs/doc_workspace/SGCP/artifacts/table1_k2_20260722/` 与外部 `C:\Workspace\2026-7-papers\infocom\SGCP\experiment\data\table1_k_sensitivity_protocol_native_20260722.csv`。
- [x] 已生成 compute 图表 addenda：`figure1b_kaware_current_protocol_ap05_gflops_20260723.*`、`figure2b_kaware_current_protocol_ap_bars_20260723.*`、`figure4b_scheduler_k_sensitivity_current_protocol_20260723.*`、`figure5b_parameter_sensitivity_gflops_current_protocol_20260723.*`、`figure8b_global_box_k_sensitivity_current_protocol_20260723.*`。caption/registry 中已明确 GFLOPs 是 detector-side forward profile，包含近似 point-feature FLOPs，不包含后处理 NMS、通信调度、CARLA 和控制计算。

## P14：2026-07-23 其他图表 K/GFLOPs 口径对齐

目的：在 Table 1 完成后，将其他核心图表同步到相同实验口径：attentive checkpoint、`40 MHz / 10 target subchannels / 60 ms`、PCS `div4/radius4/min128/r35`、EdgeCooper `35 m / member budget 3 / grid budget 200`、`K=1/2` receiver concurrent-inbound sensitivity，并补齐 GFLOPs 数据列。

- [x] Table2/6 no-clustering + global box aggregation：重跑 PCS/EdgeCooper K=1/2，并计入 box-sharing 通信量和 GFLOPs。结果文件：`C:\Workspace\2026-7-papers\infocom\SGCP\experiment\data\table6_global_box_k_sensitivity_current_protocol_20260723.csv`；图：`figure8b_global_box_k_sensitivity_current_protocol_20260723.png/.pdf`。
- [x] Table3 same-coalition scheduler comparison：重跑 PCS/EdgeCooperHD K=1/2，并计入 inter-cluster box 通信量和 GFLOPs。结果文件：`C:\Workspace\2026-7-papers\infocom\SGCP\experiment\data\table3_scheduler_k_sensitivity_current_protocol_20260723.csv`；图：`figure4b_scheduler_k_sensitivity_current_protocol_20260723.png/.pdf`。
- [x] Table4 / parameter sensitivity：2026-07-23 已按 clean package 当前主点重新完成 41 帧感知实验，而不是沿用旧 strict/default trace。固定 attentive、40MHz、10 target subchannels、PAPG、coalition-game、inter-cluster NMS、`N_max=4/rho_th=3/head_rb_budget=2`，新增 `rho_th`、`N_max`、target-subchannels 和 raw-LiDAR Mbps budget sweep，并补齐 raw/box/total Mbps 与 GFLOPs。2026-07-24 已按用户要求移除旧 `Communication Deadline` 表，用 Raw LiDAR Mbps Budget 完全替代。结果文件：`docs/doc_workspace/SGCP/artifacts/parameter_sensitivity_current_protocol_20260723/`，clean package 文档：`C:\Workspace\2026-7-papers\infocom\SGCP\experiment\06_parameter_sensitivity.md`。
...
- [x] 按用户要求统一分簇侧和调度侧 COV 公式：grid-level `C=q_i(1-q_r)`、`O=q_i`、`V=q_i * 1[q_r>0]`，vehicle-to-coalition utility 由同一 grid 公式求和得到；去除 `0.35/0.55` 等难以写入论文的经验系数。
...
- [x] 为论文证明需求新增 exact potential-verified coalition admission variant：`potential_verified_cov_coalition_game.py`。该版本将迁移提交条件增强为 source/target affected potential increment `Delta Phi_C > 0`，可证明每个 accepted migration 满足 `J_i^C > 0` 并保证有限 partition space 下终止。旧 leave-one-out `Phi_C` 结果为 `0.84/0.74/0.31`；改为 pairwise multi-view `Phi_C` 并对齐正式 SGCP-CV 迭代顺序后，41 帧 current-protocol 恢复到 `0.87/0.79/0.35`、raw `59.48 Mbps`，可作为 proof-compatible variant。
