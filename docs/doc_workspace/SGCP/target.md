...
- [ ] 新增 detector/pre-NMS 诊断：针对 failure_diagnostics 中 “nearest head got dense points but no final box” 的 35 行，导出每个相关 head 的 pre-NMS boxes、scores、best IoU、source CAVs 与目标 grid 点级/box 内点数，区分 detector 无框、late fusion/NMS 抑制和 grid 内点云未落到目标实体三类原因。
