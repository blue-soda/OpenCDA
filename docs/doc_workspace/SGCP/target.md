...
- [x] 执行真实 CARLA+NS3 有限 tick 短回归：已新增并更新 `online_ns3_short_regression.md`。首次回归发现 `vehicles_num=1` 过早初始化导致 NS3 address collision / SIGABRT；已修复为车辆注册完成 gate。修复后 35 tick 在线回归 OpenCDA exit 0、NS3 20 车初始化、38/38 sync_ack、无 sync timeout、无 fatal、无 manual reject，在线 CP AP 为 0.86/0.84/0.74。
- [ ] 分析真实在线 NS3 中的大包分片与 PHY decode failure：修复后在线短回归仍有 PSCCH/PSSCH decode failures 和部分 incomplete upload。第一轮日志诊断发现 `PotentialGame.clear_resource_allocation_strategy()` 未清空各 CAV `ClusteringScheduler.channel_allocation`，导致在线多轮 CP 残留旧链路，新旧链路可能在同一 receiver/subchannel 上重叠；已补清理逻辑，下一步需重跑真实在线短回归确认 PSCCH/PSSCH failure 是否下降。
