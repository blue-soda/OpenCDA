...
- [ ] 补 `perception_aware_potential_game` 真实 NS3 socket replay/在线短回归：当前已完成 11 帧 dry-run upload plan，需启动 NS3 后验证 scheduled request application/RLC complete、manual subchannel 无 reject、PHY failure 为 0 或可解释，并确认 deadline-aware 在线 CP 口径与离线主表差异。
