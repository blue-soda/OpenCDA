...
- [ ] 分析真实在线 NS3 中的大包分片与 PHY decode failure：修复后在线短回归仍有 PSCCH/PSSCH decode failures 和部分 incomplete upload，需要把 online request、application callback、RLC completion、PHY failure 对齐，判断是容量/并发压力、分片策略还是 online scheduler 语义差异。
