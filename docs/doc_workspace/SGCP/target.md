...
- [ ] 用真实 CARLA + NS3 或离线 NS3 replay 验证时间同步和 transfer request 语义：CARLA tick / OpenCDA network time slot / NS3 sync time 必须一致；带宽内无冲突请求成功，带宽外请求延迟或丢包。已完成 default 10/5 子信道回归，以及 `spatial_diverse` 10/20 子信道 replay：10 子信道 110/110 request application/RLC complete、44 条未调度需求跳过；20 子信道 154/154 request application/RLC complete；PHY failures 均为 0。后续可补真实 CARLA+NS3 短回归。
...
- [ ] 修正主表 baseline 口径：FullPerception-RSU 在当前 RSU-free dump 上不可直接填实测，应以 full 20-CAV early `0.85/0.83/0.48`、60,838,528 bytes 作为 centralized FullPerception upper reference；Random/MWS scheduler 因 payload 过低只作 w/o PPS 消融；公平主对比使用 payload-matched selective sharing（高预算 0.80/0.76/0.40，37,710,864 bytes）和 SGCP spatial-diverse 10/20ch。
- [ ] 补充通信量可调参数实验：已完成 `spatial_diverse` `rho_th=1/2/3/4` sweep，其中 `rho_th=3.0` 为 0.79/0.76/0.38，payload 29,405,296 bytes；后续可决定主表采用 `rho_th=2.0` 低通信点还是 `rho_th=3.0` 更高 AP 点。
