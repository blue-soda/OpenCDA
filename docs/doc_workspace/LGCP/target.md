...
- [ ] Neural feature slicing / model-level hierarchy：已完成 PointPillar `intermediate_attentive` feature geometry probe、真实 feature crop / slice manifest smoke、Top-23 首帧 leader-local feature fusion smoke、RSU feature assembly smoke、detection-head probe 和 reference-frame alignment diagnostic；当前 reference-frame coverage ratio `0.065263`、postprocess 18 boxes，但 mean abs yaw delta `93.41 deg`，仍需 feature rotation / affine warp 才能报告有效 AP，或收窄为 feature-level coverage/byte proxy。
