...
- [ ] Neural feature slicing / model-level hierarchy：已完成 PointPillar `intermediate_attentive` feature geometry probe、真实 feature crop / slice manifest smoke、Top-23 首帧 leader-local feature fusion smoke 和 RSU feature assembly smoke；当前 RSU canvas coverage ratio `0.033161`、max overlap `16`；下一步接 detection head / postprocess 或定义 feature-level coverage/byte proxy。
