...
- [ ] 实现 LGCP 专用 RSU area assignment、leader local fusion 和 RSU global aggregation 管线。（已完成 offline assignment / upload plan、hierarchy area-budget sweep、raw LiDAR feature-slice budget sweep、raw-slice-aware upload plan dry-run / 3 帧与 11 帧 request-level NS3 trace、single-slot scheduled NS3 smoke、multi-slot scheduling proxy、3 帧 live multi-slot replay、leader/RSU aggregation proxy、model-level hierarchy 入口审计、box-level late-fusion adapter smoke、Top-30 1 帧完整 area run、Top-30 3 帧 run、Top-30 11 帧 run、baseline 对齐表、common-byte/raw-byte 对照、PointPillar feature geometry probe、真实 feature crop export smoke、leader-local feature fusion smoke、RSU feature assembly smoke、detection-head probe、reference-frame alignment diagnostic、nearest/bilinear coordinate-warp smoke、AP probe、neural feature proxy summary、reference-aligned point-slice RSU BEV attentive fusion 11 帧 planned-area run、sparse training sample export、dataset helper、model wrapper、1-frame train smoke、Top-5 train5/val6 train smoke、Where2comm checkpoint + LGCP external area-mask route、`area ∩ objectness` selector、Top-10/Top-23 scale-boundary smoke、leader-once second-hop accounting、Where2comm per-leader box hierarchy diagnostic、Where2comm leader feature packet RSU fusion；有效 Top-5 intermediate/leader-packet AP 已出现，但 Top-23 仍需 RSU feature aggregation 微调、metadata overhead、多场景验证）
  - 2026-07-28 OpenCOOD 原文复现线最新进展：`leader_box` 已修复非 ego leader 坐标方向和严格 area filter 的 TP 丢失问题，默认一圈 guard-band 后，多 leader 自然分组在 OPV2V 5-CAV / 20-frame slice 上数值正常。`Delta_g=0.10` 达到 AP@0.3/0.5/0.7 `0.905693/0.902392/0.854416`、`2.579593 Mbps`、`43.106x` edge-assisted reduction，接近原文 `44x` 口径。
  - 2026-07-29 OpenCOOD 原文机制对齐修正：用户指出 Top-K BEV area selection 不是 LGCP 原文机制，已新增 `--lgcp_original_area_feature_mask`，使 `leader_box` 实际 feature mask 与 `Delta_g=0.10` 选出的 `area_groups` 完全一致，并与 `--lgcp_area_mask` 互斥。OPV2V 5-CAV / 167-frame 严格 group-area mask 结果为 AP@0.3/0.5/0.7 `0.795933/0.791619/0.694981`、`1.988045 Mbps`、`47.794x` reduction；mask keep ratio 仅 `0.7758%`，说明真实按 `10m x 6m` area 裁中期特征会产生明显 recall loss。此前 Top-K mask 结果只能保留为 Where2comm 探索，不再作为原文 LGCP 机制复现证据。
  - 2026-07-29 larger-area/overlap rescue：已新增米级 overlap 参数并完成 OPV2V 5-CAV sweep。`20m x 12m + 8m overlap` 在 167 帧上达到 AP@0.3/0.5/0.7 `0.872071/0.868043/0.781997`，接近 no-actual-mask `0.889357/0.886544/0.827220`，但通信为 `19.907312 Mbps`、reduction 仅 `3.952x`。结论是 overlap 可以救 AP，但无法同时保留原摘要 `44x` 通信优势；论文应将其作为真实 area-masked feature fusion 的 Pareto/limitation，而不是主通信优势证据。
  - 2026-07-29 larger-area/small-overlap sweep：已补跑用户点名的 `10m x 6m + 1m`，167-frame AP@0.3/0.5/0.7 为 `0.810602/0.806211/0.710632`、`30.213x`。当前“较大 area + 较小 overlap”最佳平衡点是 `25m x 15m + 1m/2m`，167-frame AP@0.5 为 `0.852906/0.854296`，reduction 为 `16.434x/13.735x`；`35m x 21m + 2m` AP@0.5 提升到 `0.861531`，但 reduction 降到 `10.730x`。下一步论文主表可采用 `25x15+1m` 或 `25x15+2m` 作为真实 area-masked feature fusion 的 Pareto 主点。
  - 2026-07-28 OpenCOOD area-mask 线最新进展：Where2comm `3x4` BEV area grid + Top-3 area constraint 达到 AP@0.3/0.5/0.7 `0.904974/0.902937/0.848870`，通信量从 `425.548300 Mbps` 降到 `208.649222 Mbps`。同一 mask 机制已扩展到 CoSDH，Top-3 达到 `0.837550/0.835279/0.766961`，通信量从 `7.403200 Mbps` 降到 `4.011488 Mbps`。下一步应扩大到更长 OPV2V 切片 / 多场景，并继续寻找可套用 generic mask adapter 的 PointPillar Max/Att/CoAlign 类 checkpoint。
  - 2026-07-28 167-frame validation 最新进展：OPV2V 5-CAV 场景段 `423-589` 已跑完。`leader_box Delta_g=0.10` 为 `0.889357/0.886544/0.827220`、`2.200203 Mbps`、`43.106x`；Where2comm Top-3 area mask 为 `0.846327/0.844633/0.789813`、`160.382871 Mbps`，baseline 为 `0.874003/0.872776/0.821696`、`312.840226 Mbps`；CoSDH Top-3 area mask 为 `0.861586/0.848816/0.743772`、`4.633623 Mbps`，baseline 为 `0.869307/0.856879/0.759882`、`7.416263 Mbps`。下一步应优先做 OPV2V 多场景 / 多 slice 验证，并为 `point_pillar_infocom` 或其他 Att/Max BEV 模型补专用 LGCP area-tile mask adapter。
  - 2026-07-29 other-model extension：已按同一 `leader_box + Delta_g=0.10 + 25m x 15m + 1m overlap` 严格 group-area feature mask 协议扩展到 V2X-ViT 与 CoAlign-w/o-box-align。V2X-ViT 167-frame no-mask 为 `0.785449/0.762059/0.653262`，mask 为 `0.763181/0.749789/0.676781`、`5.693814 Mbps`、`19.047x`；CoAlign 远端缺 stage-1 box-align JSON，但修正 `resnet: true` 后 w/o-box-align 可运行，no-mask 为 `0.956021/0.851802/0.528447`，mask 为 `0.933200/0.819052/0.497706`、`2.061053 Mbps`、`39.151x`。`pointpillar_attentive_fusion` baseline AP 为 `0/0/0`，暂不作为主结果；camera CoBEVT 缺 `corpbevt` 入口和 OPV2V camera BEV 数据，不能进入当前 3D box AP 表。
