...
- [ ] 实现 LGCP 专用 RSU area assignment、leader local fusion 和 RSU global aggregation 管线。（已完成 offline assignment / upload plan、hierarchy area-budget sweep、raw LiDAR feature-slice budget sweep、raw-slice-aware upload plan dry-run / 3 帧与 11 帧 request-level NS3 trace、single-slot scheduled NS3 smoke、multi-slot scheduling proxy、3 帧 live multi-slot replay、leader/RSU aggregation proxy、model-level hierarchy 入口审计和 box-level late-fusion adapter smoke；Top-30 多帧 late-fusion 与 neural feature slicing + model-level fusion 仍待实现）
