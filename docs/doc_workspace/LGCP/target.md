...
- [ ] Neural feature slicing / model-level hierarchy：已完成 PointPillar `intermediate_attentive` feature geometry probe 和真实 feature crop / slice manifest smoke；当前可导出 `scatter` 与 `fused` `.npz` slices，Top-23 首帧 float16 压缩后 `810.69KB`、约 `35.25KB/area`；下一步实现 leader local feature fusion 和 RSU global aggregation。
