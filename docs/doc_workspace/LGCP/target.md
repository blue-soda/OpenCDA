...
- [ ] Neural feature slicing / model-level hierarchy：已完成 PointPillar `intermediate_attentive` feature geometry probe、真实 feature crop / slice manifest smoke、Top-23 首帧 leader-local feature fusion smoke、RSU feature assembly smoke 和 detection-head probe；当前 `psm/rm` 与 postprocess 可运行，但 RSU canvas 仍是 index-space assembly，尚未跨 leader 做 world/RSU-frame 对齐；下一步实现坐标对齐或收窄为 feature-level coverage/byte proxy。
