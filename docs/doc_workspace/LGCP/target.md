...
- [ ] Neural feature slicing / model-level hierarchy：已完成 PointPillar `intermediate_attentive` feature geometry probe、真实 feature crop / slice manifest smoke、Top-23 首帧 leader-local feature fusion smoke、RSU feature assembly smoke、detection-head probe、reference-frame alignment diagnostic、nearest/bilinear coordinate-warp smoke、AP probe 和 neural feature proxy summary；当前 nearest AP@0.5 `0.010000`、AP@0.7 `0.000000`，bilinear AP@0.5 `0.011364`、AP@0.7 `0.003472`，均不能作为论文级模型性能结果；且未优化 feature crop 为 raw member area23 均值的 `16.89x`，不能宣称 feature slicing 自动省通信；下一步只能二选一：做 affine/grid-sample warp + feature calibration / retrained aggregation，或在论文中收窄为 feature-level feasibility / byte boundary。
