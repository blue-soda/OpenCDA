...
3. 若继续扩展 ablation，应优先把当前 single-slot scheduled smoke 扩展为多 slot / latency-aware LGCP scheduler，而不是只增加 random seed。
