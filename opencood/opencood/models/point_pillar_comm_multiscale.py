...
class PointPillarCommMultiscale(nn.Module):
    ...
    def __init__(self, args):
        ...
        if is_resnet:
            if ResNetBEVBackbone is None:
                raise ModuleNotFoundError(
                    'ResNetBEVBackbone is required when resnet=True.')
    ...
    def forward(self, data_dict):
        ...
        if not ego_flag and not self.training:
            ...
            if self.use_dir:
                ...
                output_dict.update({'dir_preds': dm, 'dm': dm})
        ...
        external_comm_mask = data_dict.get("external_comm_mask")
        external_comm_recon = data_dict.get("external_comm_recon")
        ...
        for i, fuse_module in enumerate(self.fusion_net):
            ...
            if self.compression:
                payload_channels = max(1, payload_channels // self.compression_ratio)
                feature_i = self.naive_compressor_list[i](feature_i, use_fp16=not self.training)
            x_out, comm_rate = fuse_module(
                feature_i,
                psm_single,
                record_len,
                normalized_affine_matrix,
                req_mask,
                external_comm_mask=external_comm_mask,
                external_comm_recon=external_comm_recon if i == 0 else None,
            )

            fused_feature_list.append(x_out)
            communication_rates.append(comm_rate)
            _, _, H_i, W_i = feature_i.shape
