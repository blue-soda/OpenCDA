...
import math
from opencda.customize.core.clustering.clustering_algorithm_manager import *
...
class PCS(ClusterResourceAllocationAlgorithm):
    ...
    def _get_vehicle_blind_spots(self, vid: int, min_division: int=1) -> Dict[int, Set[str]]:
        ...
        while unassigned_grids:
            ...
            spot_id += 1
        print(f"vid: {vid}, blind_spot_grids: {len(blind_spot_grids)}, blind_spots: {len(blind_spots)}")
    ...
    def _generate_potential_links(self, min_division: int=1, min_overlap: int=20):
        ...
        print(f"self.all_links: {len(self.all_links)}")
